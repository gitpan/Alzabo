...
foreach my $db (@db)
{
	...
	"Schema name should be 'foo_$db' but it's " . $s->name );
    ...
    eval { my @t = $s->tables( qw( bartab baztab ) ) };
    ok( ! $@,
	"Attempt to retrieve 2 tables at once threw an exception: $@" );

    eval { $s->move_table( table => $other,
			   before => $s->table('bartab') ) };
    ok( ! $@,
	"Attempting to move the 'other' table before the 'bartab' table threw an exception: $@" );
    ...
    ok( $order_ok,
	"The move_table method did not actually move the tables" );

    eval { $other->move_column( column => $other->column('somethin'),
				before => $other->column('other_pk') ) };
    ok( ! $@,
	"Attempting to move the 'somethin' column before the 'other_pk' column threw an exception: $@" );
    ...
    ok( $order_ok,
	"The move_column method did not actually move the columns" );

    eval { $tbi->set_name('newname') };
    ok( ! $@,
	"Unable to change the name of the 'tbi' column to 'newname': $@" );

    eval { $s->make_table( name => 'YAtable',
			   before => scalar $s->table('other') ) };
    ok( ! $@,
	"Attempt to give a 'before' param in call to make_table threw exception: $@" );

    eval { $other->make_column( name => 'othertest',
				type => 'int',
