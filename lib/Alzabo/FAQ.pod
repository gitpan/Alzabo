...
C<Alzabo::ObjectCache::Store::Memory> module has no limit on how much
memory it will use.  You may want to simply set the maximum requests
for each Apache child to a lower number.  Or you could call the
C<< Alzabo::ObjectCache->clear >> method from time to time.  Or you
could also simply use a different storage module, like
C<Alzabo::ObjectCache::Store::BerkeleyDB>.
